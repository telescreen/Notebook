...
%  single random number of the specified TYPE and parameters A ndB.
%
...
% 'salt & pepper' Salt and pepper numbers of aamplitude 1 with probability
%               Pb = B. the default values are Pa = Pb = A = B = 0.01. 
%               Note that the noise has values 0 (with probability Pa = A) 
%               and 1 (with probability Pb = B), so scaling is necessary 
%               if values other than 0 and 1 are required. The noise matrix
%               R is assigned three values. If R(x,y) = 0, the noise at
%               (x, y) is pepper (black). if R(x,y) = 1 the noise at (x,y)
...
% 'rayleigh'    Rayleigh noise with parameters A and B. The default values
%               are A = 0 and B = 1.
% 'exponential' Exponential random numbers with parameter A. The default is
%               A = 1.
...
switch lower(type)
    ...
    case 'erlang'
        ...
        if (b ~= round(b) || b <= 0)
            error('Param b must b a positive integer for Erlang')
        end
        k = -1/a;
        R = zeros(M,N);
        for j = 1:b
